...
*   **`http_request`**: Use for interacting with external APIs, such as checking live shipping statuses from a tracking ID. **This tool is restricted to `https://webhook.site/...` URLs.** If another host is required, summarize what you need instead of calling the tool.
